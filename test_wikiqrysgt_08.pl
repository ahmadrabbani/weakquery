...
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/run.run";
...
foreach (@lines){
	...
	if (length($page_id) >0 && $page_id =~ /\d+/) {
		...
		if ($query ne 'arizona game and fish' && $query ne 'consumer on-line shopping' && $query ne 'Sheep and Wool Festival' && $query ne 'letting india into the club'){
...
sub wikiDisambiguation {
	...
	if (keys(%overlap_link_withcontext)>0){
		...
		&outputWSDresult($link_result, $topic, $qn, "$outd_gg/topic", $count_result, $context, \%sense_as_def);
		...
		my $topic_hit_dir = "$outd_gg/topic";    # for topic expansion
		
		if ( !-e $topic_hit_dir ) {
			my @errs = &makedir( $topic_hit_dir, $dirmode, $group );
			print "\n", @errs, "\n\n" if (@errs);
		}
		
		#get top k expansion terms, and merge with original topic
		my $expand_terms_ref = &getLCA_gg(lc($topic), \%gg_lca_terms, $qn, $topic_hit_dir);
		push (@terms, @$expand_terms_ref);
...
sub segByMI {
	...
	if (length($page_id) >0 && $page_id =~ /\d+/){
		...
		if (length($an_to_id) > 0 && $an_to_id =~ /\d+/ ){
			...
			print LOG scalar(@w1) . ' ' . scalar(@w2) . ' ' . $match . "\n\n";

			# mi_value = freq(w1,w2)*MI(w1,w2)
			my $mi_value = 0.0;
			if ($match > 0){
				$mi_value = $match * log( (scalar(@google_words) * $match) / (scalar(@w1) * scalar(@w2)) ) / log(2);
			}
...
sub outputWSDresult {
	...
	if (keys(%tf_withcontext) > 0){
		my @keys = sort {$tf_withcontext{$b} <=> $tf_withcontext{$a}} keys %tf_withcontext;
		print LOG "definition_result: shift(@keys)";
	}
...
sub getLCA_gg{
	...
	sleep 1;
	#my $quotated_topic = '"' . $topic . '"';
	#my $htmlcnt = &queryGoogle2( $quotated_topic, $hit_dir, $qn, $maxdN);
