...
#            modified rerankrt13.pl (07/2008)
#              - %rrsc is output every 1000 records rather than 250
#              - outputs consolidated rrsc files
#            modified rerankrt13fix.pl (11/2008)
#              - uses updated opinion lexicons
#              - uses updated opinion scoring subroutine 
#            modified rerankrt13new.pl (11/2008)
#              - documents w/o exact query title string match is penalized
#              - uses updated opinion scoring subroutine (fixed more bugs than rerankrt13new.pl)
#            modified rerankrt13new2.pl (11/2008)
#              - documents w/o exact query title string match is penalized
#              - uses updated opinion scoring subroutine (fixed more bugs than rerankrt13new.pl)
...
# OUTPUT:    
#   $ddir/results(_new)/$qtype/trecfmt(x)/$arg3.R$arg8/rset10/$runname-$arg4.1
#     -- results w/ topic reranking scores
#        QN docID RANK RT_SC RunName Topic_SCs (per line)
#   $ddir/results(_new)/$qtype/trecfmt(x)/$arg3.R$arg8/rset10/$runname-$arg4.2
#     -- results w/ opinion reranking scores
#        QN docID RANK RT_SC RunName Opinion_SCs (per line)
#   $ddir/results/$arg3/tmp/rrsc10$arg8(x)/$arg2-$runname-$arg4.1
#     -- new qry-independent scores file
#        docID SCOREs (per line)
#   $ddir/results/$arg3/tmp/rrsc10$arg8(x)/$arg2-$runname-$arg4.2
...
#   3. outputs consolidated rrsc files
# ------------------------------------------------------------------------
...
my ($log,$logd,$sfx,$noargp,$append,@start_time,@start_time2);
...
my $subj1f= "$ddir/lex2/final2/W1.lex";          # strong subj. lexicon
my $subj2f= "$ddir/lex2/final2/W2.lex";          # weak subj. lexicon
my $empf=   "$ddir/lex2/final2/EM.lex";          # emphasis lexicon
my $IUlexf= "$ddir/lex2/final2/IU.lex";          # IU lexicon file
my $HFlexf= "$ddir/lex2/final2/HF.lex";          # HF lexicon file
my $LFlexf= "$ddir/lex2/final2/LF.lex";          # LF lexicon file
my $LFrgxf= "$ddir/lex2/final2/LFrgx.lex";       # LF regex file
my $LFmrpf= "$ddir/lex2/final2/LFmrp.lex";       # LF morph regex file
my $AClexf= "$ddir/lex2/final2/AC.lex";          # AC lexicon file
...
my $poutcnt= 1000;     # rrsc output interval
...
require "$pdir/blogsub2.pl";     # blog subroutine library
...
my $outd= "$r0d/rset10";         # !!rerank result file directory
...
if (-e "$r0d/rall8" && !$nosafety) {
    print STDOUT "$r0d/rall8/ already exist. Cannot continue\n";
    #exit;
}
...
foreach my $file(@files) {
    ...
    while(<IN>) {
        ...
        my ($qn,$docID,$rnk,$sc,$run);
        if (/^(\d+).+(BLOG06-200\d+-\d{3}-\d+)\s+(\d+)\s+([\d\.\-]+)\s+(.+)$/) {
            ($qn,$docID,$rnk,$sc,$run)=($1,$2,$3,$4,$5);
        }
        else {
...
if (!$rrscnewf) {
    my @flist1=&mkRRSChash($outd2,\%rrsc,$qtype,\%rrlist);
    my @flist2=&mkRRSChash($outd2tmp,\%rrsc,$qtype,\%rrlist);
...
foreach my $docID(keys %dlist) {
    ...
    if (exists($dlist2{$docID})) {
        ...
        # after every 1000 records have been processed,
        #   1. save %rrsc to file
        ...
        if ($pcnt && $pcnt%$poutcnt==0) {
            ...
            if ($log) { @start_time2= &start_clock("RRSC READ $readcnt"); }
            ...
            if ($log) { &stop_clock("RRSC READ $readcnt","",@start_time2); }
        ...
        #-----------------------------------------------------
        my %score;
        &calrrSCall9($title,$body,\%qti,\%qti2,\%qti3,\%qds2,\%ph,\%ph2,\%nrph,\%nrn,\%score,\%AClex,\%HFlex,\%IUlex,\%LFlex,
...
foreach my $file(keys %result0) {
    ...
    foreach my $qn(sort keys %{$result0{$file}}) {
        foreach my $rank(sort {$a<=>$b} keys %{$result0{$file}{$qn}}) {
            ...
            my @scs;
            foreach my $name('ti','bd','ti2','bd2','tix','bdx','bdx2','ph','ph2','nrph','nrn','qtixm') {
                if ($rrsc{$docID}{$qn}{$name}) {
                    if ($name eq 'qtixm') {
                        push(@scs,sprintf("%.0f",$rrsc{$docID}{$qn}{$name}));
                    }
                    else {
            ...
            if ($rrsc{$docID}{0}{'pav'}) {
                ...
                push(@scs,sprintf("%.4f",$ol_pse/($ol_pse+$ol_npse)));
            }
            else {
                push(@scs,sprintf("%.4f",0));
            }
...
# arg3 = query type
# arg4 = pointer to %rrlist
# r.v. = list of files read
#-----------------------------------------------------------
sub mkRRSChash {
    my($ind,$rrhp,$qtype,$rrlp)=@_;
    ...
    foreach my $file(@files) {
        ...
        foreach (@lines) {
            ...
            if ($ftype) {
                ...
                next if (!exists($rrlp->{$docID}));  # process only IDs in the result files

                next if (!defined($avsc));  # skip incomplete lines (being written by other processes)
            ...
            # ti2, bd2: match of individual query terms
                ...
                next if (!defined($avx));  # skip incomplete lines (being written by other processes)
...
sub printRRSC {
    ...
    my $outf1= "$dir/$qtype-$rsubd-$rsetnum.1";  # qry-independent scores
    my $outf2= "$dir/$qtype-$rsubd-$rsetnum.2";  # qry-dependent scores
    ...
    foreach my $docID(keys %$rrlhp) {
        ...
        foreach my $qn(sort {$a<=>$b} keys %{$rrschp->{$docID}}) {
            ...
            if ($qn==0) {
                ...
                #!! 11/16/2008: 1 if exact query title string found in body text
                if ($rrschp->{$docID}{0}{'qtixm'}) {
                    push(@scs,sprintf("%-7s","QTIXM=1"));
                }
                else {
                    push(@scs,sprintf("%-7s","QTIXM=0"));
                }
