...
"arg3= result file name prefix (optional)\n";
...
foreach my $file(@files) {
    ...
    next if ($file !~ /top3f|base4|wdoqsBase/);
...
sub mkRRf {
    ...
    foreach my $qn(sort {$a<=>$b} keys %result) {
        ...
        my (%result1,%result2,%result3,%result4,%result5,%result6,%result7,%rest,%sc,%scdiff,%score)=();
                ...
                foreach my $wtn(1) {
                    foreach my $pol('N','P') {
                        ...
                        if ($pol eq 'N') {
                            ...
                            if ($rank<=100) {
                                if ($scdiff{$pol}>=0.1) {
                                    $result1{$k}{$docno}=$score{$k}; 
                                }
                                else {
                                    $result2{$k}{$docno}=$score{$k}; 
                                }
                            }
                            elsif ($rank<=250) {
                                if ($scdiff{$pol}>=0.1) {
                                    $result3{$k}{$docno}=$score{$k}; 
                                }
                                else {
                                    $result4{$k}{$docno}=$score{$k}; 
                                }
                            }
                            elsif ($rank<=$maxrank) {
                                if ($scdiff{$pol}>=0.1) {
            ...
            foreach my $wtn(1) {
                foreach my $pol('N','P') {
                    ...
                    if ($pol eq 'N') {
                        ...
                        ($rank,$score,$oldsc)=&rrgrp2($OUT,$result7{$k},$result0{$k},$qn,$rank,$score,$oldsc,$rest{$k});
                    }

                    else {
                        ($rank,$score,$oldsc)=&rrgrp2($OUT,$result2{$k},$result0{$k},$qn,$rank,$score,$oldsc,$rest{$k});
                    }
