...
# 3.  - exclude runs already executed
#   Kiduk, 6/11/08
#   modified old runtime2.pl
...
my $xdir= join(', ',sort keys %done);


$logs= "/u3/trec/blog07/200*/indxblog1.pl.log";
$div= "#-----------------------------------------";

$outf= "indx$stype"."allsub.sh";

open(OUT,">$outf") || die "can't write to $outf";
$head=&head("indxblog$stype",$xdir);
print OUT "$head\n";
