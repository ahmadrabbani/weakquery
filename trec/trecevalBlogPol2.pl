...
if ($qtype=~/^t/) {
    %qrels= (
        'N'=>"$qreld/qrels.negative.blog06-07",
        'P'=>"$qreld/qrels.positive.blog06-07",
    );
}
else {
    %qrels= (
        'N'=>"$qreld/qrels.negative.blog08",
        'P'=>"$qreld/qrels.positive.blog08",
    );
}
...
my $odir= "$ddir/$resultd/$qtypes{$qtype}/eval/$qsubd"."POL0";
...
foreach my $file(@files) {
    ...
    my $err1= `$evalc -q -M1000 -c $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_topic`;
    print LOG "CMD1=$evalc -q -M1000 -c $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_topic\n";
    print LOG "Error ($err1): CMD1" if ($err1);
    my $err2= `$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion`;
    print LOG "CMD2=$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion\n";
    print LOG "Error ($err2): CMD2" if ($err2);

    $ftype='P';
    $file2=$file.$ftype;

    $err1= `$evalc -q -M1000 -c $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_topic`;
    print LOG "CMD1=$evalc -q -M1000 -c $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_topic\n";
    print LOG "Error ($err1): CMD1" if ($err1);
    $err2= `$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion`;
    print LOG "CMD2=$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion\n";
    print LOG "Error ($err2): CMD2" if ($err2);
