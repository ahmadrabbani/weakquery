...
# -----------------------------------------------------------------
# Name:      trecevalBlogP.pl
# Author:    Kiduk Yang, 06/13/2008
#              modified trec07/treceval.pl (11/2007)
# -----------------------------------------------------------------
# Description:  evaluate TREC results
#   - compute polarity performance and output to another directory
#     (used to assess the polarity performances of non-polarity runs)
# -----------------------------------------------------------------
...
# Input:     $idir/$arg1/results/$qtype/trecfmt/$arg2/*
# Output:    $idir/$arg1/results/$qtype/eval/$arg2P/*
...
my $odir= "$ddir/$resultd/$qtypes{$qtype}/eval/$qsubd"."P";
...
foreach my $file(@files) {
    ...
    my $err1= `$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion`;
    print LOG "CMD2=$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion\n";
    print LOG "Error ($err1): CMD2" if ($err1);

    $ftype='P';
    $file2=$file.$ftype;

    $err1= `$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion`;
    print LOG "CMD2=$evalc -q -M1000 -c -l2 $qrels{$ftype} $idir/$file > $odir/$file2$fsfx\_opinion\n";
    print LOG "Error ($err1): CMD2" if ($err1);
