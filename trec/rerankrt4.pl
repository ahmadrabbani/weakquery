...
#            arg6= input file name (optional)
# INPUT:     $ddir/results/$qtype/trecfmt(x)/$arg2.R/rall7/*.2
#              -- results w/ opinion reranking scores
#            $ddir/results/$qtype/trecfmt(x)/$arg2.R/*-$arg3.r1
#              -- topic reranked results
# OUTPUT:    $ddir/results/$qtype/trecfmt(x)/$arg2.R1/$arg3.r2
...
"arg4= number of docs to rerank (optional: default=500)\n".
"arg5= 1 to rerank results beyond maxrank in a group (optional)\n".
"arg6= result file name prefix (optional)\n";
...
my ($arg_err,$qtype,$rsubd,$fwtf,$maxrank,$rrlow,$fpfx)= chkargs($prompt,\%valid_args,3);
die "$arg_err\n" if ($arg_err);
...
my $rdir= "$ddir/results/$qtype{$qtype}/trecfmt/$rsubd"."R";
...
foreach my $file(@files) {
    ...
    my $outfa= "$outf-a.r2";  # all lex weight
    ...
    print "writing to $outfa\n\n";
    
    &mkRRf($inf,$inf2,$outfm,$outfc,$outfp,$outfa);
...
sub mkRRf {
    my ($tpinf,$opinf,$outm,$outc,$outp,$outa)=@_;
    ...
    while(<TPIN>) {
        ...
        my ($qnum,$docno,$rank,$sc,$tgroup,$run)=split/\s+/;
        ...
        $sc{'in1'}=$iucnt1 if ($iucnt1);
        $sc{'in2'}=$iucnt2 if ($iucnt2);
        $sc{'av'}=$av if ($av);
        $sc{'avx'}=$avx if ($avx);
        $sc{'tlen'}=$tlen if ($tlen);
    ...
    my %OUT;
    open($OUT{'a'},">$outa") || die "can't write to $outa";
    open($OUT{'m'},">$outm") || die "can't write to $outm";
    open($OUT{'c'},">$outc") || die "can't write to $outc";
    ...
    foreach my $qn(sort {$a<=>$b} keys %result) {
        ...
        #  - %result4: rest
        #  - key=docno, val=score
        my(%result1,%result2,%result3,%result4,%result5,%result6,%result7,%result8)=();
        ...
        foreach my $line(@{$result{$qn}}) {
            ...
            if (@opsc) {
                ...
                foreach my $name(@vnames) { 
                    # non-polarity scores
                    if ($name!~/[NP]/) { 
                        $rr_sc{'a'} += $fswt{$name}*$sc_norm{$name}; 
                        if ($name !~ /_[cp]/) { $rr_sc{'m'} += $fswt{$name}*$sc_norm{$name}; }
                        elsif ($name !~ /_[mp]/) { $rr_sc{'c'} += $fswt{$name}*$sc_norm{$name}; }
            ...
            foreach my $k('a','m','c','p') {
                ...
                #  !! try $group? !!
                if ($rank<=$maxrank) {
                    if ($group=~/A[12]/) { $result1{$k}{$docno}=$score; }
                    elsif ($group=~/A/) { $result2{$k}{$docno}=$score; }
                    elsif ($group=~/B[12]/) { $result3{$k}{$docno}=$score; }
                    elsif ($group=~/B/) { $result4{$k}{$docno}=$score; }
                    elsif ($group=~/C[12]/) { $result5{$k}{$docno}=$score; }
                    elsif ($group=~/C/) { $result6{$k}{$docno}=$score; }
                    else { $result7{$k}{$docno}=$score; }
                }
        ...
        foreach my $k('a','m','c','p') {
            ...
            ($rank,$score,$oldsc)=&rrgrp($OUT,$result5{$k},$result0{$k},$qn,$rank,$score,$oldsc);
            ($rank,$score,$oldsc)=&rrgrp($OUT,$result6{$k},$result0{$k},$qn,$rank,$score,$oldsc);
            ($rank,$score,$oldsc)=&rrgrp($OUT,$result7{$k},$result0{$k},$qn,$rank,$score,$oldsc);
            ($rank,$score,$oldsc)=&rrgrp($OUT,$result8{$k},$result0{$k},$qn,$rank,$score,$oldsc);
