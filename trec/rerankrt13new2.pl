...
# OUTPUT:    
#   $ddir/results(_new)/$qtype/trecfmt(x)/$arg3.R$arg8/rset9/$runname-$arg4.1
#     -- results w/ topic reranking scores
#        QN docID RANK RT_SC RunName Topic_SCs (per line)
#   $ddir/results(_new)/$qtype/trecfmt(x)/$arg3.R$arg8/rset9/$runname-$arg4.2
#     -- results w/ opinion reranking scores
#        QN docID RANK RT_SC RunName Opinion_SCs (per line)
#   $ddir/results/$arg3/tmp/rrsc9$arg8(x)/$arg2-$runname-$arg4.1
#     -- new qry-independent scores file
#        docID SCOREs (per line)
#   $ddir/results/$arg3/tmp/rrsc9$arg8(x)/$arg2-$runname-$arg4.2
...
my $outd= "$r0d/rset9";         # !!rerank result file directory
...
foreach my $docID(keys %dlist) {
    ...
    if (exists($dlist2{$docID})) {
        ...
        #-----------------------------------------------------
        my %score;
        &calrrSCall8($title,$body,\%qti,\%qti2,\%qti3,\%qds2,\%ph,\%ph2,\%nrph,\%nrn,\%score,\%AClex,\%HFlex,\%IUlex,\%LFlex,
