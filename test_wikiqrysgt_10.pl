...
# Ver2.1	8/29/2011
#	Revise result: only the top topic is wrapped with double quotation, its context not regardless of multiple or single word
#	if any of the ngram is a Wikipedia title or anchor, then it is followed by : plus Wikipedia id, such as:
#	"registration renewal" bell_county:22562196 bell county internet
#	"bell county:180846" internet registration renewal
#	"Sheep  Wool" Festival:67500
#	As 9/8/2011
#	remove query stopword before sending to segmentation, to match current annotation as: "Radio Waves" and "Brain Cancer"
#	with several exceptions
#	-- not valid anymore
#	Not remove stopwords in query for sub segmentation and mysql(could use Uppercase), only use stop and stem in gg_count and gg_sigma
#	should compare query segmentation accuracy remove vs. not remove stopword 
#	when remove stopword in the run, queries in the annotate should also remove stopword to calculate accuracy
# Ver 3.0 11/2/2011
#	Add subroutines to experiments
#	1. unigram topic model
#	2. WSD: - print all senses; if context is a Wikipedia topic, print three sense representation and score in log in addition;
#	3. WSD context terms by LCA 
#	getLCA_gg
#	outputWSDresult
#	calPti_MLE
# Ver 3.1 11/11/2011
#	Not check whole query is a Wikipedia topic
# --------------------
...
#		getContentById: $id is a Wikipedia page id or an_to
...
my $lambda = 0.3;
my $MI_threshold = 0;	#blog
my $run_name = 'blog';
#my $MI_threshold = 100;	#hard
#my $run_name = 'hard';
#my $run_name = 'wt09';
...
#my $input_file = "$home_dir/Desktop/dissertation/data/segments/segments.greater4.test";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics_qn.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics_qn_pti.run";
my $input_file = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics_qn_ambiguous.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/hard05.50.topics.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/hard05.50.topics_qn.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/hard05.50.topics_qn_pti.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/hard05.50.topics_qn_ambiguous.run";
#my $input_file = "$home_dir/Desktop/dissertation/data/topics/wt09.topics.full.run";
...
foreach (@lines){
	...
	#}
		...
		my $outd_gg = "$gg_hits_dir/$run_name/q$qn";
		print LOG "outd_gg:" . $outd_gg . "\n\n";
		if ( !-e $outd_gg ) {
		...
		#9/8/2011
		#not remove stopword by query
		#as checking whether the whole query is a Wikipedia topic at the beginning, only several queries need specical treatment
		if ($query ne 'arizona game and fish' && $query ne 'consumer on-line shopping' && $query ne 'Sheep and Wool Festival'){
			#remove white space trailing the stopword
			$query =~ s/\bthe\b\s+?|\bon\b\s+?|\band\b\s+?|\bfor\b\s+?|\bin\b\s+?|\bof\b\s+?//g;
			$query =~ s/\bthe\b\s+?|\bon\b\s+?|\band\b\s+?|\bfor\b\s+?|\bin\b\s+?|\bof\b\s+?//g;
		}
		...
		if ( scalar(@words) == 1 ) {
			...
			print LOG "\nStarting query segmentation ranking with segByMI--- ", timestamp(0), "\n\n";
			$result = &segByMI( $query, $stem_query, \@google_stem_docs, \@google_stem_words, $MI_threshold ) . '::' . $qn;
		}
		elsif ( scalar(@words) > 2 ) {
			...
			print LOG "\nStarting query segmentation ranking with rankSeg--- ", timestamp(0), "\n\n";
			...
			$result= &rankSeg( $ngrams_ref, $contexts_ref, $dirichlet_smooth, $lambda, \@google_stem_words, \@google_stem_docs, $qn, $outd_gg) . '::' . $qn;
		}
	#}	#end if-else
...
sub getPageid {
	...
	#print "page mysql_topic: " . $mysql_topic . "\n";
...
sub getAntoid {
	...
	#print LOG "an_to mysql topic: " . $mysql_topic . "\n\n";
...
#-----------------------------------------------------------
#  arg1 = Wikipedia an to
#  r.v. = the content to the an to
#-----------------------------------------------------------
sub getContentById {
	my $an_to = shift(@_);
	#print LOG "Enter getContentById:" . ' ' . $an_to . "\n\n";
	my $query = "SELECT co_content FROM content WHERE co_id=" . "'" . $an_to . "'";
	my $sth = $dbh->prepare($query) || die "cannot prepare statement:" . $dbh->errstr();
	$sth->execute;
...
sub rankSeg {
	...
	my ( $ngrams_ref, $contexts_ref, $dirichlet_smooth, $lambda, $google_words_ref, $google_docs_ref, $qn, $outd_gg)
	  = @_;
		...
		if ($isTopic){
			...
			if(length($page_id) > 0 && $page_id =~ /\d+/){
				...
				if (&chkAmbiguation($topic)){
					$page_id = &wikiDisambiguation($topic, $contexts{$topic}, $qn, $outd_gg);
				}
				else {
			...
			# calculate pti with MLE
			$pti = &calPti_MLE($topic, $google_words_ref);
		}
	...
	foreach my $top_topic (@top_topic){
		...
		#result: "new york:id" times_square:id building
		# parse the result by splitting white space, top topic wrapped with dovuble quotation
		my @context_terms = split(/" /, $top_topic_context);
		foreach my $term (@context_terms){
			#print LOG "original term:" . $term . "\n\n";
			if ($term =~ /^"|"$/){
				$term =~ s/^"|"$//;
			}
			#print LOG "term:" . $term . "\n\n";
...
sub wikiDisambiguation {
	...
	while((my $id, my $count) = $sth->fetchrow_array){
		...
		$sense_as_def{$id} = &getContentById($id);
		#print LOG "wikiDisambiguation context: " . &getDefById($id) . "\n\n";
		push (@an_to, $id);
	}
	...
	if (keys(%overlap_link_withcontext)>0){
		...
		my @link_keys = sort {$overlap_link_withcontext{$b} <=> $overlap_link_withcontext{$a}} keys %overlap_link_withcontext;
		my $link_result = shift(@link_keys);
		
		my @count_keys = sort {$sense_as_count{$b} <=> $sense_as_count{$a}} keys %sense_as_count;
		my $count_result = shift(@count_keys);
		
		#output result with three sense representation
		&outputWSDresult($link_result, $topic, $qn, $outd_gg, $count_result, $context, \%sense_as_def);
		...
		#get top k expansion terms, and merge with original topic
		my $expand_terms_ref = &getLCA_gg(lc($topic), \%gg_lca_terms, $qn, $outd_gg);
		my @expand_terms = @$expand_terms_ref;
		print LOG "add number of " . scalar(@expand_terms) . " words to context\n\n";
		push (@terms, @expand_terms);
		
		#disambiguate by context occurrence in an_to definition and expansion terms
		foreach my $term (@terms){
			my $tf_count = 0;
			$term =~ s/^"||\s"//;
		...
		if (keys(%tf_withcontext) > 0){
			...
			my $context_result = shift(@keys);
			print LOG "definition_result_wikiDisambiguation: $context_result" . "\n\n"; 
			...
			my $count_result = shift(@keys);
			print LOG "count_result_wikiDisambiguation: $count_result" . "\n\n";
			return $count_result;
		}
	}
...
sub calPti {
	...
	foreach my $word (@words){
		...
		if (defined($page_word_count) && $page_word_count > 0){
			$mle_page *= $page_word_count/$page_table_count;
		}
		if (defined($anchor_word_count) && $anchor_word_count > 0){
...
sub calConditionalProb {
	...
	print LOG "prob:" . $prob . ", $dirichlet_smoothing\n\n";
	
	#$prob = ($count_n + 1500 * $pti) / ($sigma_n + $dirichlet_smoothing);
	#print LOG "prob:" . $prob . ", 1500 \n\n";
	
	#$prob = ($count_n + 5000 * $pti) / ($sigma_n + $dirichlet_smoothing);
	#print LOG "prob:" . $prob . ", 5000 \n\n";
...
#  arg1 = bigram
#  arg2 = stem bigram
#  arg3 = google all stem docs array
#  arg4 = MI threshold
...
sub segByMI {
	...
	my ( $bigram, $stem_bigram, $google_stem_docs_ref, $google_stem_words_ref, $threshold ) = @_;
	...
	if (length($page_id) >0 && $page_id =~ /\d+/){
		...
		if (length($an_to_id) > 0 && $an_to_id =~ /\d+/ ){
			...
			my ( $ori1, $ori2 ) = split( / /, $bigram );
			
			#print LOG "w1, w2, ori1, ori2:" . ' ' . $w1 . ',' .$w2 . ',' . $ori1 . ',' . $ori2 . "\n\n";
			...
			my @google_stem_docs = @$google_stem_docs_ref;
			my @google_stem_words = @$google_stem_words_ref;
			my $big_line = join (' ', @google_stem_words);
			
			my $match = 0;
			
			foreach my $doc (@google_stem_docs){
				my @doc_words = split(/ /, $doc);
				for (my $i = 0; $i < scalar(@doc_words); $i += 200){
					my $str = substr($doc, $i, 199);
					#print LOG $str . "\n\n";
				...
				#my @doc_words = split(/ /, $doc);
				#for ( my $i = 1 ; $i < scalar(@doc_words) ; $i += 20 ) {
				#	my $str = join( ' ', @doc_words[ $i..$i+19 ] );
				#	print LOG "$str" . "\n\n";
				#	$match++ if ( $str =~ /\b$w1\b/i && $str =~ /\b$w2\b/i );
				#	
				#	my ($ori_w1, $ori_w2) = split(/ /, $bigram);
			...
			my @ori_w1 = $big_line =~ /\b$ori1\b/gi;
			my @ori_w2 = $big_line =~ /\b$ori2\b/gi;
			
			print LOG scalar(@ori_w1) . ' ' . scalar(@ori_w2) . ' ' . $match . "\n\n";

			# mi_value = freq(w1,w2)*MI(w1,w2)
			my $mi_value = 0.0;
			if ($match > 0){
				my $MI_part1 = scalar(@google_stem_words) * $match;
				my $MI_part2 = scalar(@ori_w1) * scalar(@ori_w2);
				$mi_value = $match * (log($MI_part1/$MI_part2)/log(2));
			}
			print LOG "MI value:" . $mi_value . "\n\n";
	  		if ($mi_value > $threshold){
				$bigram_seg = '"' . $bigram . '"' . ' ';
			}
			else {
				$bigram_seg = '"' . $ori1 . '"' . ' ' . '"' . $ori2 . '"' . ' ';
			}
		}
	}
...
sub getMysqlAnchorTopic {
	...
	#print LOG "topic1: topic2" . $topic1 . ' ' . $topic2 . "\n\n";
	...
	# anchor could have more than one link to pages!!!
	if ($count >= 1){
		return $topic;
	}
	else {
...
#-----------------------------------------------------------
#  print all senses to log, this sub is called in wikiDisambiguation
#  as an additional step with if overlapcount > 0
#------------------------------------------------------------
#  arg1 = link result
#  arg2 = ambiguous topic
#  arg3 = qn
#  arg4 = search result dir to topic search: $outd_gg/topic
#  arg5 = count result
#  arg6 = context
#  arg7 = reference to sense_as_def 
#  r.v. = 
#-----------------------------------------------------------
sub outputWSDresult {
	my ($link_result, $topic, $qn, $result_dir, $count_result, $context, $sense_as_def_ref) = @_;
	
	my %sense_as_def = %$sense_as_def_ref;
	
	print LOG "\n\n Ambiguous topic: $topic\n\n";
	print LOG "link_result_outputWSDresult: $link_result\n";
	...
	if ( !-e $result_dir ) {
		my @errs = &makedir( $result_dir, $dirmode, $group );
		print "\n", @errs, "\n\n" if (@errs);
	}
	
	#get top k expansion terms, and merge with original topic
	my $expand_terms_ref = &getLCA_gg(lc($topic), \%gg_lca_terms, $qn, $result_dir);
	my @expand_terms = @$expand_terms_ref;
	print LOG "add number of " . scalar(@expand_terms) . " words to context\n\n";
	...
	foreach my $term (@terms){
		...
		$term =~ s/^"||\s"//;
		foreach my $an_to (keys %sense_as_def){
			#print LOG "content:" . $sense_as_def{$an_to} . "\n\n";
			$tf_count += &calTf($term, $sense_as_def{$an_to});
			$tf_withcontext{$an_to} = $tf_count if $tf_count > 0;
		}
	}
	...
	if (keys(%tf_withcontext) > 0){
		my @keys = sort {$tf_withcontext{$b} <=> $tf_withcontext{$a}} keys %tf_withcontext;
		my $result_definition = shift(@keys);
		print LOG "definition_result_outputWSDresult: $result_definition\n";
	}
	else {
		print LOG "definition_result_outputWSDresult: null!!!\n";
	}
	
	print LOG "count_result_outputWSDresult: $count_result\n";
...
#-----------------------------------------------------------
#  calculate topic language model with MLE at word level
#  this sub is called by raknkSeg as alternative 
#  to calPti separately
#------------------------------------------------------------
#  arg1 = topic
#  arg2 = reference to Google word array
#  r.v. = topic probability
#-----------------------------------------------------------
sub calPti_MLE {
	print LOG "Enter calPti_MLE...\n\n";
	my ($topic, $word_ref) = @_;
	
	my @words = @$word_ref;
	my $word_str = join(' ', @words);
	
	my $pti = 1.0;
	
	my @topics = split(/ /, lc($topic));
	
	foreach my $word (@topics){
		my @match = $word_str =~ /\b$word\b/gi;
		#my @match = $word_str =~ /$word/gi
		if (scalar(@match) >0){
			$pti = $pti * (scalar(@match)/scalar(@words));
		}
		else {
			$pti = 0;
			last;
...
#-----------------------------------------------------------
#  get expansion term with Google LCA
#  this sub is called in wikiDisambiguation to enrich
#  query context
#------------------------------------------------------------
#  arg1 = ambiguous topic
#  arg2 = term weight hash
#  arg3 = qn
#  arg4 = search result dir to topic search: $outd_gg/topic
#  r.v. = top expansion terms
#-----------------------------------------------------------
sub getLCA_gg{
	...
	&LCA("$hit_dir", \%qrywd, $maxdN, $qeterm_hash_ref, \%stoplist);
