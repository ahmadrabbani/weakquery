...
# Input:
#	$query::$qn
#	$qn for AOL will be the query string (s/ /_/)
# Output:
...
#	  $qn for AOL will be the query string (s/ /_/)
# Result:
#	"$sgt1" "sgt2" ... "sgti"::$qn if ther is valid segmentation
#	OR
#	$query::$qn if otherwise
# --------------------
# Use P(ti|Cti) to estimate the most appropriate query segmentation
# could also produce the "central topic"
# e.g., P("new york"|"time square" building) is higher than 
#	P("time square"|"new york" building)
# indicates that "new york" is the central query topic 
# --------------------
# ver1.0	3/17/2011
...
# start segmentation
# $result = "$sgt1" "sgt2" ... "sgti"::$qn where each segmentation is wrapped with double quote
my $result = '';
...
#TODO: add query Wkipedia page id
...
sub rankSeg {
	...
	#hash key: ngram if a Wikipedia topic
	#value: ngram's Wikipedia page_id
	my %page_id;
	...
	foreach my $topic ( keys %contexts ) {
		...
		my $sigma_n = &gg_count_sigma( $topic, $google_docs_ref );
		...
		if ($isTopic >= 1){
			...
			if ($flag) {
				...
				$page_id{$topic} = $wk_stem_page_id{$topic};
			}
		}
		else {
			# calculate pti
	...
	my $top_topic = shift(@ranked_topic);
	
	my $top_topic_context = $contexts{$top_topic};
	
	my $context_result = '';
	# context can be mixed with ngram wrapped with quotation and single word
...
sub gg_count {
	...
	my (@context_ngrams, $context_word);
	my $co_count = 0;
	...
	foreach my $doc (@google_stem_docs){
		if ($doc =~ /\b$topic\b/i){
			if (index($context, '"') != -1){
				@context_ngrams = split(/\" /, $context);
				# single word context is behind ngram context
				# based on sub segmnentation
				
				# get single word context
				$context_word = pop(@context_ngrams);
				
				my @context_words = split(/ /, $context_word);
			
				foreach my $word (@context_words) {
					if ($doc =~ /\b$word\b/i) {
						# co-occurrence between topic and each context word
						$co_count += 1.0;
					}

				}
				
				foreach my $ngram (@context_ngrams){
					if ($doc =~ /\b\$ngram\b/i) {
						#ngram weigth
						$co_count += 2.0
					}
...
sub segmentation {
	...
	foreach my $ngram (@ngrams) {
		...
		if ( $result == 0 ) {
			...
			# check whether the $remain_qry contains any existing ngrams
			foreach my $another (@ngrams) {
				if ( index( $remain_qry, $another ) != -1
					&& $another != $ngram )
				{
					$contexts{$ngram} .= '"' . $another . '"' . ' ';
					$remain_qry =~ s/\b$another\b//;
				}
			}
