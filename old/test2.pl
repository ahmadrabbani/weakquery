...
#-------------------------------------------------
	...
	my $htmlcnt = &queryGoogle2($q, $outd, $qn, $maxhtmN);
	...
	my $status= &queryWikipedia($q,$outd1,$qn);
	...
	if ($n == 2) {
		$qryseg_str = &segByMI($q, $outd2, $MI_threshold);
		print "bigram query: $q is segmented as: " . "$qryseg_str" . "\n";
	}
	elsif ($n == 1) {
		print "query with only one word is unprocessable!\n"; 
	}
	elsif ($n > 2) {
		print "starting processing a multi-word query: $q...\n";
		# step #1
        # bisects
		...
		if ($did == 0) {
			...
			$qryseg_str = $q;
			print "multi-gram query: $q is segmented as a whole: " + "$qryseg_str" . "\n";
		}
		elsif ($did == 1) {
