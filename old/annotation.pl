...
my $blog_topic = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics";

my $blog_output = "$home_dir/Desktop/dissertation/data/topics/06-08.all-blog-topics.annotation";
...
open(IN, "<$blog_topic") || die "cannot open $blog_topic\n";
while(<IN>){
	chomp;
    next if (/^\s*$/);
	push (@lines, $_);
...
my $qcnt = 0;
my $acnt = 0;
open (OUT, ">$blog_output") || die "cannot write to $blog_output\n";
foreach (@lines) {
