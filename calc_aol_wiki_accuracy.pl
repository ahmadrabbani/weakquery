...
my $result_dir = "$home_dir/workspace/epic_prog/query/result/without_stop";

my $anote_dir = "$home_dir/Desktop/dissertation/data/segments";

my $result_file = "$result_dir/great4.test.rsl";
my $anote_file = "$anote_dir/segments.greater4.test";
#my $anote_file = "$anote_dir/segments.greater4.test.annotator2";
#my $anote_file = "$anote_dir/segments.greater4.test.annotator3";
my $total_qry = 500;
...
#hash key: qn
#value: precisionat segment level 
my %seg_precision;
...
foreach my $line (@result){
	...
	if ($q_accu == 1.0){
		...
		print "!!!incorrect:" . $target . ' with:' . $anote . "\n\n";
	}
...
sub calqaccu {
	...
	if($result =~ /^"(.*?):\d+"$/){
		print "1:" . $1 . ' ' . $anote . "\n";
		#Wikipedia topic or anchor is a valid topic
		return 1.0;
		#my $word = '"' . $1 . '"';
		#if ($word eq $anote){
		#	return 1.0;
					...
					if ($r =~ /(.*?_.*?_?.*?):(\d+)/){
						...
						if(exists($anotes{$word})){
							$correct_count++;
								...
								if (exists($anotes{$ngram})){
									$correct_count++;
									#TODO: what about ngram that has more than two words
									$num_segments--;
								}
						}
						$num_segments++;
