...
if (length($page_id) >0 && $page_id =~ /\d+/) {
	...
	if ( scalar(@words) == 1 ) {
		...
		$result = &segByMI( $query, \@google_stem_words, $MI_threshold ) . '::' . $qn;
	}
	elsif ( scalar(@words) > 2 ) {
...
sub gg_count {
	...
	print LOG "original topic: $topic" . ' ' . "stem topic: $stem_topic\n";
	...
	print LOG "original context: $context" . ' ' . "stem context: $stem_context\n";
	...
	foreach my $doc (@google_stem_docs){
		...
		if ($doc_has_allcontext){
			my $tf_topic = $doc =~ /\b$stem_topic\b/i;
			$co_count += $tf_topic;
		}
	}
...
sub wikiDisambiguation {
	...
	while((my $id, my $count) = $sth->fetchrow_array){
		...
		$sense_as_links{$id} = &getPagelinkIn($id);
...
sub segByMI {
	...
	my ( $bigram, $google_stem_words_ref, $threshold ) = @_;
	...
	if (length($page_id) >0 && $page_id =~ /\d+/){
		...
		if (length($an_to_id) > 0 && $an_to_id =~ /\d+/ ){
			...
			my $mi_value =
	  		$match * log( scalar(@google_words) * $match / ( scalar(@w1) * scalar(@w2) ) ) /	log(2);
